...
def parse_path_file(filename, dataset, dataset_path, triples, triple_id_to_subgraph_dict, reverse):
	...
	while line_id < len(lines):
		...
		triple_id_to_subgraph_dict[test_triple_id] = subgraph_edges
...
if __name__=='__main__':
	...
	map_size = len(triples) * BYTES_PER_DATUM * 20 * 400
	...
	for test_triple_id in tqdm(triple_id_to_subgraph_dict):
		edges = triple_id_to_subgraph_dict[test_triple_id]
		...
		if args.reverse:
			datum = {'n1' : n2, 'n2' : n1, 'r_label': r_label, 'nodes': nodes, 'edges': edges}
		else:
