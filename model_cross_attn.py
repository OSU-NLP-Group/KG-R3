...
class GraphTransformer(nn.Module):
	...
	def __init__(self, n_entities, n_relations, args):
		...
		config_cross_attn = BertConfig(0, hidden_size=self.embed_dim,
							num_hidden_layers=3,
		...
		self.encoder_query_cross_attn = BertEncoder(config_cross_attn)
		self.encoder_query_cross_attn.config = config_cross_attn
		
		self.layer_norm_query_cross_attn = nn.LayerNorm(self.embed_dim, eps=1e-12)

		# projection layer for final ranking
	...
	def init_weights(self):
		...
		self.encoder_query_cross_attn.apply(partial(BertPreTrainedModel._init_weights, self.encoder_query_cross_attn))
	...
	def forward(
		...
		inputs_embeds = self.subgraph_self_attn_encoder_layer_norm(inputs_embeds)

		# transform attention mask for input to subgraph self-attention encoder
		extended_attention_mask_subgraph = attention_mask[:, None, :, :]
		extended_attention_mask_subgraph = (1.0 - extended_attention_mask_subgraph) * -10000.0

		# compute subgraph self-attention encoder representations
		subgraph_embed_hidden_states = self.subgraph_self_attn_encoder(inputs_embeds, attention_mask=extended_attention_mask_subgraph, n_entity_nodes=n_entity_nodes, return_dict=False)[0] # [batch, subgraph_seq_len, hid_dim]
		...
		extended_attention_mask_query = (1.0 - attention_mask.unsqueeze(1)) * -10000.0

		# compute query self-attention encoder representations
		query_embed_hidden_states = self.self_attention_encoder_query(query_embed, attention_mask=extended_attention_mask_query, return_dict=False)[0] # [batch, query_seq_len, hid_dim]		
		query_embed_hidden_states = F.dropout(query_embed_hidden_states, p=0.6, training=self.training)
		query_embed_hidden_states = self.layer_norm_query_cross_attn(query_embed_hidden_states)
		
		# compute cross-attention encoder representations		
		query_embed_cross_attn = self.encoder_query_cross_attn(query_embed_hidden_states, encoder_hidden_states=subgraph_embed_hidden_states, attention_mask=extended_attention_mask_query, return_dict=False)[0][:, 0] # [batch, hid_dim]
		
		# compute final logits
		subgraph_embed_cross_attn = subgraph_embed_hidden_states[:, 0]
		final_embed = self.W(torch.cat([query_embed_cross_attn, subgraph_embed_cross_attn], dim=-1)) # [batch, hid_dim]
